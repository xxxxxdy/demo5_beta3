...
export function CodeToString(code_message, string_length = 24){
    let str = "";
    let unit_width = CODE_WIDTH / string_length;
    let hill_num = code_message["hline"].length, hill_idx = 0, str_split = CODE_PADDING;
    /// pos: the position relative to visual code of one hill:
    ///   0: out of this hill
    ///   1: in left white part(include left side)
    ///   2: in black part(include left side)
    ///   3: in right white part(include both side)
    let le_idx = -1, ri_idx = -1, le_pos = 0, ri_pos = 0;
    if(code_message["hline"][0].x <= str_split){le_idx = 0, le_pos = 1}
    // console.log(code_message, unit_width)
    for(let i=0; i<string_length; i++){
        if(le_idx >= hill_num) {
            str+="0";
            continue;
        }
        str_split += unit_width;
        if(str_split < code_message["hline"][0].x){
            str+="0";
            continue;
        }

        let out_of_hill = false;
        while(str_split > code_message["hline"][hill_idx].x2) {
            hill_idx++;
            if(hill_idx>= hill_num){
                out_of_hill = true;
                break;
            } 
        }
        ri_idx = hill_idx;
        if(out_of_hill) ri_pos = 0;
        else if(str_split >= code_message["rect"][ri_idx].width+code_message["rect"][ri_idx].x){
            ri_pos = 3;
        }
        else if(str_split >= code_message["rect"][ri_idx].x){
            ri_pos = 2;
        }
        else if((str_split >= code_message["hline"][ri_idx].x)){
            ri_pos = 1;
        }
        else{
            ri_pos = 0;
        ...
        if(ri_idx-le_idx>=3) str+="7";
        else if(ri_idx-le_idx>=2) str+="6";
        else if(ri_idx-le_idx>=1){
            if(ri_pos===0){
                switch(le_pos){
                    case 1:
                    case 2:
                    ...
                    case 3:
                        ...
                        break;
                    default: // case 0:
                        str+="6";/// I think this situation would not occur
                        break;
                }
            }
            else if(ri_pos===1){
            ...
            if(ri_pos !== le_pos) str+="3";
            else if(ri_pos === 2) str+="2";
            else str+="1";
        }

        le_idx = ri_idx, le_pos = ri_pos;
    }
    // console.log(str)
    return str;
}
