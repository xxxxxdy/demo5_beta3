...
export class CodeData{
    ...
    buildTreeWithPersistent(j = 2){
        if(this.hill_list.length <= 1) return;

        /// sort hill from small persistent to bigger
        this.hill_list.sort(function(a, b){
            return a.end_persistent - b.end_persistent
        })
        
        let now_pers=0, i = 0, cnt = 0, hills_num = this.hill_list.length;
        while(cnt < hills_num){
            cnt ++;
            now_pers = this.hill_list[i].end_persistent;
            ...
            if(kill_left){
                ...
                if(getDataValue(this.hill_list[near_hill].left_idx, j) === this.hill_list[near_hill].min_value){
                    ...
                    new_pers = this.hill_list[near_hill].end_persistent;
                }
            }
            else{
                for(let j=i+1; j<this.hill_list.length; j++){
    ...
    getHillJsonData(i){
        ...
        let top_time = getDataTime(this.hill_list[i].top_idx);
        let right_time = getDataTime(this.hill_list[i].right_idx);
        let time_width = right_time - left_time;
        let persistent = this.hill_list[i].end_persistent;
        for(let j=this.hill_list[i].upper; j !== -1; j=this.hill_list[j].upper){
            persistent = this.hill_list[j].end_persistent;
        }
        if(persistent === Infinity) persistent = this.max_value-this.min_value;
        ...
        return {
            ...
            'top_time':(top_time - left_time)/(right_time - left_time),
            ...
            'persistent': persistent/this.value_range
        }
    }

    getDataForOneDraw(persistent = 0){
        var data_list = [];
        
        for(let i=0; i<this.hill_list.length; i++){
            // There may be any other filter condition if necessary
            if(this.hill_list[i].start_persistent <= persistent && this.hill_list[i].end_persistent > persistent){
